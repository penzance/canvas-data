...
public class AssignmentDim implements DataTable {
  ...
  private Timestamp peerReviewsDueAt;
  ...
  public AssignmentDim(final TableFormat format, final CSVRecord record) throws ParseException {
    ...
    if ($peerReviewsDueAt != null && $peerReviewsDueAt.length() > 0) {
      this.peerReviewsDueAt = Timestamp.valueOf($peerReviewsDueAt);
    }
    ...
    if ($allDayDate != null && $allDayDate.length() > 0) {
      this.allDayDate = format.getDateFormat().parse($allDayDate);
    }
  ...
  /**
   * Current workflow state of the assignment.  Possible values are 
   * unpublished, published and deleted 
   */
   ...
   */
  public Timestamp getPeerReviewsDueAt() {
