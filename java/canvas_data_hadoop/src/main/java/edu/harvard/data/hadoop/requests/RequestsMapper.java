...
public class RequestsMapper extends Mapper<Object, Text, LongWritable, ExpandedRequest> {
  ...
  public void map(final Object key, final Text value, final Context context) throws IOException, InterruptedException {
    ...
    for (final CSVRecord csvRecord : parser.getRecords()) {
      Requests request;
      request = new Requests(format, csvRecord);
      final ExpandedRequest expanded = new ExpandedRequest(request);
      parseUserAgent(expanded);
      if (request.getCourseId() == null) {
        context.write(new LongWritable(-1L), expanded);
      } else {
        context.write(new LongWritable(request.getCourseId()), expanded);
      }
    }
  }

  private void parseUserAgent(final ExpandedRequest request) {
